...
public class CrawlerService {
    ...
    public void test() {
        ...
        movie.setName("test2Movie");
        movieDAO.save(movie);
        System.out.printf(movie.getId().toString());
    }
    ...
    private Record getOneRecordToCrawl() {
        Record r = recordDAO.getFirstByCrawled(0);
        if (r == null) {
            r = new Record();
        ...
        if (el != null) {
            ...
            for (Element item : items) {
                if (item.getElementsByClass("fold-bd").size() < 1 && item.children().get(1).getElementsByTag("p").size() > 0) {
                    ...
                    LOG.info("Comment for 《" + movie + "》:" + comm.getContent());
